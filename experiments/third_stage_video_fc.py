...
from models.third_stage_video_fc import ThirdStageFlowFC
from models.third_stage_video_fc_conditional import ThirdStageFlowFCConditional
from data.datamodule import StaticDataModule
# from utils.metrics import compute_fvd

class ThirdStageVideoModelFC(Experiment):
    def __init__(self,config,dirs,devices):
        ...
        if ("conditional" in self.config["general"]) and self.config["general"]["conditional"]:
            model = ThirdStageFlowFCConditional
        else:
            model = ThirdStageFlowFC

        if self.config["general"]["restart"] or self.config['general']['test'] != 'none':
            ckpt_path = self._get_checkpoint()
            self.model = model.load_from_checkpoint(ckpt_path,map_location="cpu",config=self.config,strict=False,dirs=self.dirs)
        else:
            self.model = model(self.config,dirs=self.dirs)
        # basic trainer is initialized in parent class
        # self.logger.info(
        #     f"Number of trainable parameters in model is {sum(p.numel() for p in self.model.parameters() if p.requires_grad)}"
        # )

        self.ckpt_callback = self.ckpt_callback(filename='{epoch}-{val-EE_R3:.3f}',monitor='val-EE_R3',
                                                save_top_k=self.config["logging"]["n_saved_ckpt"], mode='min', period=1)
    ...
    def train(self):
        ...
        trainer.fit(self.model,datamodule=datamod)
    ...
    def test(self):
        ...
        # logger = get_logger()
        #### extraction accuracy for endpoint error and angular error

        if self.config['general']['test'] == 'accuracy' or self.config['general']['test'] == 'realism':
            print('***************************COMPUTING METRICS FOR EACH INDIVIDUAL NUMBER OF POKES  ******************************************')
            # self.model.console_logger.info('***************************COMPUTING METRICS FOR EACH INDIVIDUAL NUMBER OF POKES******************************************')
    ...
    #             self.model.console_logger.info('***************************COMPUTING METRICS OVER SUMMARIZED POKES*******************************************************')
    ...
    #             for count,n_pokes in enumerate(tqdm(reversed(range(max_n_pokes)),desc='Conducting metrics experiment....')):
    #                 self.config['data']['n_pokes'] = n_pokes +1
    #                 self.model.console_logger.info(f'Instantiating {n_pokes + 1} pokes...')
    #
    #                 datamod = StaticDataModule(self.config["data"], datakeys=self.datakeys, debug=self.config['testing']['debug'])
    #                 datamod.setup()
    #                 trainer = self.basic_trainer(limit_test_batches=n_test_batches)
    ...
    #
    #             kps_dict =self.model.metrics_dict['KPS']
    #             # ssim_dict = self.model.metrics_dict['SSIM']
    #             # lpips_dict = self.model.metrics_dict['LPIPS']
    ...
    #
    #             df_acc = pd.DataFrame.from_dict(self.model.metrics_dict)
    #
    #
    ...
    #             df_acc.to_csv(os.path.join(self.model.metrics_dir, f'plot_data_{n_samples_per_poke}pokes_acc_metrics-{postfix}.csv'))
    #
    #             # metrics only reported when gt keypoints are available
    #             if len(kps_dict) > 0:
    #                 df_kps = pd.DataFrame.from_dict(kps_dict)
    #                 fig_savename = os.path.join(self.model.metrics_dir, f'keypoint_err_plot_{n_samples_per_poke}samples-{postfix}.pdf')
    #                 df_kps.to_csv(os.path.join(self.model.metrics_dir,f'plot_data_{n_samples_per_poke}pokes_kps-{postfix}.csv'))
    ...
    #                 df_kps_group.to_csv(os.path.join(self.model.metrics_dir, f'plot_data_kps_group.csv'))
    #
    #             # image based metrics which can be reported for all datasets
    #             # fig_savename = os.path.join(self.model.metrics_dir, f'ssim_plot_{n_samples_per_poke}samples-{postfix}.pdf')
    #             # df_ssim.to_csv(os.path.join(self.model.metrics_dir, f'plot_data_{n_samples_per_poke}pokes_ssim-{postfix}.csv'))
    #             # make_errorbar_plot(fig_savename, df_ssim, xid='Time', yid='Mean SSIM per Frame',
    #             #                    hueid='Number of Pokes',varid='Std per Frame')
    #             #
    #             # fig_savename = os.path.join(self.model.metrics_dir, f'lpips_plot_{n_samples_per_poke}samples-{postfix}.pdf')
    #             # df_lpips.to_csv(os.path.join(self.model.metrics_dir, f'plot_data_{n_samples_per_poke}pokes_lpips-{postfix}.csv'))
    #             # make_errorbar_plot(fig_savename, df_lpips, xid='Time', yid='Mean LPIPS per Frame',
    #             #                    hueid='Number of Pokes', varid='Std per Frame')
    #
    #             # fig_savename = os.path.join(self.model.metrics_dir, f'psnr_plot_{n_samples_per_poke}samples.pdf')
    #             # df_psnr.to_csv(os.path.join(self.model.metrics_dir, f'plot_data_{n_samples_per_poke}pokes_psnr.csv'))
    ...
    #             # df_ssim_group.to_csv(os.path.join(self.model.metrics_dir, f'plot_data_ssim_group.csv'))
    #             # self.model.console_logger.info(f'Mean ssim value from sample metric is {df_ssim_group["Mean SSIM per Frame"]}')
    #             # # df_psnr_group=df_psnr.groupby('Number of Pokes', as_index=False).mean()
    #             # # df_psnr_group.to_csv(os.path.join(self.model.metrics_dir, f'plot_data_psnr_group.csv'))
    #             # df_lpips_group = df_lpips.groupby('Time', as_index=False).mean()
    #             # df_lpips_group.to_csv(os.path.join(self.model.metrics_dir, f'plot_data_lpips_group.csv'))
    #             # self.model.console_logger.info(f'Mean lpips value from sample metric is {df_lpips_group["Mean LPIPS per Frame"]}')
    #         else:
    #             mean_divscore = torch.mean(torch.tensor(self.model.div_scores))
    #             self.model.console_logger.info(f'Diversity score averaged over all pokes is {mean_divscore}')
    #
    #     else:
    ...
    #         if self.config['general']['test'] == 'fvd':
    #             from utils.general import get_logger
    #             samples_real = np.load(os.path.join(self.model.savedir_fvd,'real_samples.npy'))
    #             samples_fake = np.load(os.path.join(self.model.savedir_fvd, 'fake_samples.npy'))
    #
