...
# Todo Modify everything for third stage. Only class name has been changed
    def __init__(self,config,dirs,devices):
        ...
        self.ckpt_callback = self.ckpt_callback(filename='{epoch}-{FVD-val:.3f}',monitor='FVD-val',
                                                save_top_k=self.config["logging"]["n_saved_ckpt"], mode='min', period=2)
