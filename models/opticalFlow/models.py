...
class FlowVAE(pl.LightningModule):
    ...
    def __init__(self,config):
        ...
        self.decoder = ConvDecoder2(self.config["architecture"]["nf_max"], decoder_channels, out_channels=self.config["architecture"]["nf_in"])
...
class ConvEncoder2(nn.Module):
    def __init__(self, nf_in, nf_max, n_stages, variational=False, norm_layer = "group", layers=None, spectral_norm=True):
        ...
        for n in range(n_stages - 1):
            for _ in range(5-n):  # additional blocks per stage
                blocks.append(
                    ResBlock(
                        nf,
                        nf,
...
class ConvDecoder2(nn.Module):
    ...
    def __init__(self,nf_in, in_channels, n_skip_stages=0, spectral_norm=True, norm_layer="group",layers=None,out_channels=3):
        ...
        for i,nf in enumerate(in_channels[1:]):
            ...
            for _ in range(3): # in forward multpiply n_stages by x from range(x)
    ...
    def forward(self,shape, del_shape=True):
        ...
        for n in range(self.n_stages*3):
