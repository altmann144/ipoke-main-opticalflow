...
class PokeMotionModelFC(pl.LightningModule):
    ...
    def __init__(self,config,dirs):
        ...
        self.first_stage_config['architecture']['min_spatial_size'] = 1 # to make 4D flow sample input
    ...
    def __initialize_first_stage(self):
        ...
        first_stage_config = path.join(*first_stage_ckpt.split('/')[:-2],'config.yaml').replace('ckpt','config')
        ...
        self.first_stage_model = FCBaselineFixed.load_from_checkpoint(first_stage_ckpt,config=self.first_stage_config,
    ...
    def __initialize_conditioner(self):
        ...
        conditioner_config = path.join(*conditioner_ckpt.split('/')[:-2],'config.yaml').replace('ckpt','config')


        with open(conditioner_config) as f:
            self.conditioner_config = yaml.load(f, Loader=yaml.FullLoader)

        self.conditioner = FirstStageFCWrapperFixed(self.conditioner_config)
        if 'restart' in self.config['general'] and not self.config["general"]["restart"]:
            ...
            # remove keys from checkpoint which are not required and remove model. from key
            state_dict = {key[6:]: state_dict["state_dict"][key] for key in state_dict["state_dict"] if
                          key[:5] == 'model'}
            # load conditioner model
            m,u = self.conditioner.load_state_dict(state_dict, strict=False)
            assert len(m) == 0, f'conditioner is missing keys {m}'
            del state_dict
    ...
    def make_flow_input(self,batch,reverse=False,use_kp_poke=False):
        ...
        with torch.no_grad():
            poke_emb = self.poke_embedder.encoder(poke)
            ...
            if self.use_cond:
                if self.conditioner.be_deterministic:
                    cond = self.conditioner.encoder(X[:,0])
                else:
    ...
    def forward_sample(self, batch, n_samples=1, n_logged_vids=1, show_progress= False, add_first_frame=False,use_keypoint_pokes=False):
        ...
        with torch.no_grad():
            ...
            for n in it:
                ...
                # flow expects and returns 4D tensor, but first stage does not if fully connected architecture
                if len(out_motion.shape) == 4:
                    out_motion = out_motion.squeeze(-1).squeeze(-1)
    ...
    def decode_first_stage(self,motion,X, length=None):
        ...
        if isinstance(self.first_stage_model,SpadeCondMotionModel):
            ...
            hidden = torch.stack([motion] * self.first_stage_config['architecture']['n_gru_layers'], dim=0)
            # x = X[:,0]

            X_hat = []

            in_rnn = torch.cat([self.first_stage_model.motion_bias] * start_frame.size(0), dim=0)[:, None]

            for i in range(X.size(1) - 1):
                out, hidden = self.first_stage_model.rnn(in_rnn, hidden)

                reaction = self.first_stage_model.gen([out.squeeze(1)], start_frame, del_shape=True)
...
class FCBaselineFixed(FCBaseline):
